...
export const createTeacher = asyncHandler(async (req, res) => {
  ...
  try {
    const teacher = await Teacher.create({ ...profile, name, email, user: user._id })
    res.status(201).json({ success: true, data: await teacher.populate('user', 'email role isActive') })
  } catch (error) {
...
export const updateTeacher = asyncHandler(async (req, res) => {
  ...
  const { password, email, name, ...profile } = req.body
  ...
  if (profile.phone !== undefined) userPatch.phone = profile.phone
  if (Object.keys(userPatch).length) await User.findByIdAndUpdate(teacher.user, userPatch, { runValidators: true })
...
export const deleteTeacher = asyncHandler(async (req, res) => {
  ...
  await Class.updateMany({ teacher: teacher._id }, { $unset: { teacher: 1 } })
